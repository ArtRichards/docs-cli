...
| 10. Quality, Docs, Refactor | Complete | 2026-05-25 | **M7 ship-ready locally; publish DEFERRED to M9 batched 1.3.0 per OQ-C.** Milestone-completion summary appended to `docs/m7-migration-accuracy.md` covering F0/F1/F4/F5/F10/F11/F12 + medium confidence + `--config-project` + sidecar narrow-refusal + open follow-ons. `docs/status.md` M7 row flipped to Complete. `CHANGELOG.md` 1.2.0 dated 2026-05-25. `python -m build` produced `dist/docs_cli-1.2.0.tar.gz` + `dist/docs_cli-1.2.0-py3-none-any.whl`; `twine check dist/*` PASSED both. **NO `twine upload`, NO `git tag v1.2.0`, NO `gh release create` — M9 rebuilds fresh post-M8 and publishes 1.3.0.** pytest still 320 / 320 GREEN; quality gate clean tree-wide. **Fresh-eyes review fixes (2026-05-25):** docs/.docs.toml `add_statuses` → `add_lifecycles` (the in-project F0 sweep missed the TOML file); README.md prose updated for `Lifecycle` + M7 ship + 1.3.0 batched-publish narrative (v1.1 in-flight wording was stale); section-header status inference tightened from `current state + progress OR updates` to `current state + (progress OR updates)` per milestone spec (bare `## Updates` no longer suffices); `_multi_project_hints` docstring simplified to match implementation (the misleading "branch" language); 4 new tests pinning the positive contracts (sidecar-allow + divergent-hint + section-header status positive/negative). pytest now **324 / 324 GREEN**. Step 3 simplify candidates surfaced by the review (in addition to those already enumerated in the milestone doc): `infer_role` mixed `str \| bool` return type from the pre-medium two-level world; test function names still containing `_status` after F0; test helpers still accepting `status=` keyword for back-compat. |
...
- [x] pytest 320 / 320 GREEN; ruff / format / mypy / docs
      ...
      (Post-review: 324 / 324 GREEN after sidecar-allow,
      divergent-hint, and section-header status positive/
      negative tests were added — review-fix commit cycle
      2026-05-25.)
