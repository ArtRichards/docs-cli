...
| 9. Implement Online/Integration | Done | 2026-06-03 | **Milestone payoff — the completed-milestone archival backlog landed on the REAL `docs/` tree.** Leg A: dogfooded all three shapes on throwaway copies (`/tmp/m18-dogfood-docs` + a full-manifest `/tmp/m18-full-dogfood`) — (1) M1 pair via `--cascade-only`, (2) sweep m13-impl beside the already-archived m13 plan (leg-2 D2 flip: archived plan's `parent-of` repointed), (3) M16 trio via `--cascade-only "m16-*"` (`--cascade-dry-run` first confirmed the glob pulls ONLY impl + test-matrix, NOT plan/status). Each ended `docs check` exit 0. Verified flag names against `docs archive --help`: `--date YYYY-MM-DD`, `--cascade-only GLOB`, `--cascade-dry-run` — all as planned. Leg B: ran the 14-op manifest in strict completion-date order, `docs check docs/` exit 0 AFTER EVERY op (no rollback needed). Landed: M1→2026-05-20, M2→2026-05-21, M3+M4→2026-05-22, M5→2026-05-23, M6→2026-05-24, M7+M8+M9→2026-05-25 (pairs via `--cascade-only "<plan>"` archiving the LOG, child-of pulls the plan); m10+m11 impl-logs swept into existing `archive/2026-05-27/`; M12 pair→2026-05-28; m13 impl swept into existing `archive/2026-05-29/`; M16 trio→2026-06-01 (`--cascade-only "m16-*"`). **Q-D leg-2 cross-milestone check PASSED:** after the M12 op, `archive/2026-05-29/m13-pypi-publish.md` reads `pairs-with: archive/2026-05-28/m12-project-rename.md`. Leg C: final `docs check docs/` exit 0; **M14/M15/M18 (both pairs each) LEFT LIVE at root** — nothing under M14/M15/M18 archived. Live referrers repointed automatically (architecture.md→m5; plan.md/status.md/release-runbook.md→archived milestones). INDEX regenerated by the ops (44 active/3 archived → 18 active/29 archived); frozen snapshot resynced in lockstep (Q6). Full suite 510 passed. **Finding (surfaced, NOT an M18 regression):** the M12 op printed a benign `could not archive m12-project-rename.md: [Errno 2]` warning — M12's impl carries BOTH `child-of:` and `pairs-with:` to the plan, so the un-deduped `_cascade_set` (pre-existing M14 machinery) yields the plan twice; the first hit archives it, the second ENOENTs and is skipped. End state correct (plan archived once, edges clean, exit 0). |
...
- Dedupe `_cascade_set` by target to silence the duplicate-edge ENOENT
  warning (the benign `could not archive … [Errno 2]` printed during the M12
  op — pre-existing M14 cascade machinery yields a doc reached by two verbs
  twice; end state is correct, exit 0). Code change deferred; not an M18
  regression.
...
rewrite); `_rewrite_referring_edges` (the `if doc.archived` skip — unconditional
at baseline, conditioned on a moving `Related:` target in Phase 6); the
`broken-ref` root-relative resolution (cli.py:1586); `_cmd_mv`
