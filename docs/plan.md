...
change existing automation. M25 is fully prepared with Phase 1 next; M26–M29
are registered draft stubs. Execution order is expressed by reciprocal
...
| M25 — Reciprocal relationship integrity and `docs relate` | **Active / milestone-setup complete** (2026-08-10; no TDD phase started; Phase 1 next) — recognized reciprocal sequence/dependency/blocker pairs, hard `missing-inverse` check errors, explicit two-endpoint add/remove repair, and narrowly audited archived-endpoint mutation. First implementation milestone in the v2.0 train. | [Plan](m25-reciprocal-relationship-integrity.md) | [Log](m25-reciprocal-relationship-integrity-impl.md) |
