...
staying `Lifecycle: active` until M29 publishes the train. With M28a landed the
train has no unmerged implementation milestone left, and **M29 — PyPI publish
2.0.0 was activated the same day** (2026-08-16, `m29/milestone-setup`): its
stub is expanded into a five-phase runbook plan, its implementation log is
open, its three activation questions are resolved as **D1–D4**, and setup's
non-mutating readiness pass (**E1–E7**) found the tree publish-ready apart from
one toolchain blocker — `twine 6.2.0` cannot check the `Metadata-Version: 2.5`
artifacts this tree now builds, which D1 fixes by upgrading to `twine>=7.0.0`.
No runbook phase has started.
...
| M29 — PyPI publish 2.0.0 | **Active — milestone setup complete 2026-08-16; no runbook phase started** (registered 2026-08-10, activated on `m29/milestone-setup` the day M28a merged; depends on M25–M28a, **all five merged to `main`**) — publish M25–M28a as one breaking safety train, verify upgrade paths against served artifacts, refresh host skills, and archive the train at closeout. **Operational, not a ten-phase code milestone**: the sections of [release-runbook.md](release-runbook.md) are its phases (M9 / M11 / M13 / M17 / M20 / M24 shape), now written out as a five-phase plan with per-phase exit criteria. Setup ran the **non-mutating** half of the runbook's Phase 1 and Phase 2 gates against the merged tree — read-only registry probes, the tree-wide quality gate, an artifact build into a scratch directory and a local-install smoke, with nothing uploaded, pushed, tagged or renamed and the repository's `dist/` left holding the shipped 1.8.0 artifacts — and recorded **E1–E7**. **Six say go:** the `2.0.0` slot is free on PyPI (released `1.3.0, 1.4.0, 1.5.0, 1.6.0, 1.6.5, 1.8.0`) and on the `docs-cli-rehearsal` detour name, whose detour continues because the bare TestPyPI name is still the unrelated squatted `0.1.0` project, unchanged since M9 (E1, E2); `~/.pypirc` is `600` with both token sections and `gh` holds `repo` (E2); the gate is green at **1504 passed** with `ruff`, `ruff format`, `mypy`, `docs check`, index idempotence, INDEX-vs-fixture and both bundled mirrors clean, and the sdist still carries no `CHANGELOG.md` (E4); **all eight v2.0 headline contracts hold against a locally built wheel** installed into a throwaway venv — `missing-inverse`, the bare `--cascade` refusal at exit 2 with the file count unchanged, `broken-body-link`, `docs mv --dry-run`'s full rewrite plan with `plan.md` still in place, the archive strand report (15 still-active inbound references on a real `--cascade-only 'm27-*'` closeout), the `Archived:` witness on **both** members of that closeout, the dated-archive-directory `mv` refusal with both dates named and the by-hand escape printed, and `archive-date-drift` on a hand-made relocation (E5); and the version SoT is `1.8.0` exactly as M25 — D6 specified, with `## UNRELEASED` still open, so M29 owns the single bump (E7). **The seventh is a blocker in the toolchain, not the package (E3):** an unpinned hatchling now emits `Metadata-Version: 2.5` and `twine 6.2.0` monkeypatches `packaging.metadata._VALID_METADATA_VERSIONS` to stop at `2.4` (`twine/package.py:32`), so `twine check` errors on both artifacts — while the installed `packaging 26.2` accepts `2.5`, **PyPI accepts `2.5`** (hatchling 1.32.0's own wheel, uploaded 2026-08-11, serves it), and **`twine 7.0.0` PASSES both**, verified in a throwaway venv. **D1** upgrades the checker to `twine>=7.0.0` at Phase 2 and explicitly **rejects** pinning hatchling backwards, which would freeze the build backend to work around a stale checker. The stub's three activation questions are RESOLVED: **D2** adopts M24's "author now, confirm at the gate" — the PyPI upload, the `main` push, the `v2.0.0` tag, the GitHub release and the issue #1 comment each stop for explicit operator authorization; **D3** fixes the closeout manifest at the five plan/log pairs plus the M29 milestone doc (eleven documents), with the M29 impl log staying active at the root on the M17/M20/M24 precedent; **D4** re-verifies the README's `blob/main/…` links **after** the closeout archive, since that archive is what can break them and M28's rewriter does not reach `README.md`. Question 1 is answered by the train itself — no intermediate versions were used, so the bump is a single `1.8.0` → `2.0.0`. Setup also found a **fourth** `README.md` item beyond the three M28 routed here: three surviving "**Claude Code** skill" mentions against **zero** in `src/docs_cli/`, M23's neutralisation never having swept the one file outside every parity gate — which is also, via `readme = "README.md"`, the PyPI project page (E6). Issue #1's prepared reply had its finding-3 paragraph rewritten against the shipped behaviour, adding the `Archived:` field name and M28a's second leg. | [Plan](m29-pypi-publish-2-0-0.md) | [Log](m29-pypi-publish-2-0-0-impl.md) |
