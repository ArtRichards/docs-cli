...
one escaping link when the milestone opened. No version bump; **M28** is next
and inherits the `BodyLink` span contract plus the guarantee that a clean tree
contains no escaping destination.
**M28's milestone setup is complete (2026-08-15)** on `m28/milestone-setup`,
...
Q1/Q2/Q3 by the operator, Q4–Q7 conductor-resolved; Phase 1 is next and does
not re-open them. Setup's headline finding is that both
...
`--report-links` as a design while adopting its output. M28a and M29 remain
