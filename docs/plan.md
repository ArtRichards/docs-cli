...
- parent-of: m23-agent-aware-install-skill.md
...
M21 (v1.7.0) is the active feature milestone — lifecycle **`draft`**, **TDD
Phases 1–4 complete (RED baseline 2026-06-29)** on branch `m21/phases-1-4`
(scaffolded 2026-06-12, **re-scoped to CLI-only 2026-06-29**; Phase 5 next). It
...
v1.7:  M21 (update-check notification — CLI new-version notice)  →  [publish TBD]
v1.8:  M23 (agent-aware install-skill + recorded-dest skill-refresh hint)  →  [publish TBD]   (recommended bump; restores the skill nudge cut from M21)
