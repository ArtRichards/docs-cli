...
3 deliberately removed, 114 new), the dogfood on a throwaway copy of this
tree, and the simplify-and-close pass; the same-instance audit added one
failure-path lock, so the suite stands at **889**. It stays `Lifecycle: active` until the
M29 closeout. M27–M29 remain registered draft stubs. Execution order is expressed by reciprocal
