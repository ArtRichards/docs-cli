...
`Lifecycle: active` until M29 publishes the train. **M26 is now in flight** —
milestone setup completed 2026-08-12 on `m26/milestone-setup`, with a full
ten-phase task plan, an implementation log, and **all seven setup questions
resolved** before Phase 1. **Step 1 (Phases 1–4) is complete** (2026-08-12 /
2026-08-13 on `m26/phases-1-4`): the contract is frozen in `cli.md`,
`convention.md`, and the milestone's *Decisions (Phase 1 — BINDING)* section,
the RED suite and the four `archive-*` fixture trees are in, and the
classified RED baseline is **873 collected, 93 failed, 780 passed** with 769
of the 777 pre-existing ids mechanically proven still GREEN. Phase 5 opens
Step 2. M27–M29 remain registered draft stubs. Execution order is expressed by reciprocal
