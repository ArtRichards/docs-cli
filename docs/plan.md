...
| M28 — Move-safe Markdown body-link rewrites | **Registered draft** (2026-08-10; no log/phase started) — reuse M27's scanner and M26's move planning to rebase incoming links and links inside moved referrers for `mv`/archive. Scope extended 2026-08-15 from `feedback-log.md` issue #1 finding 1: a post-plan strand-check refusing any move that would leave a still-active document pointing at a newly-archived one. Depends on M26 + M27. | [Plan](m28-move-safe-body-link-rewrites.md) | _not yet created_ |
