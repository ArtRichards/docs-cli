...
auto-resolved, the rest conductor-resolved. **Phase 1 — Define Contract
completed 2026-08-15** on `m28a/phases-1-4`, freezing both legs as items
(A)–(H) of *Decisions (Phase 1 — BINDING)* with five amendments to
setup-frozen material and nine Step-1 resolutions (OQ-1 … OQ-9, OQ-7 an
operator decision); Phase 2 — Write Tests (RED) is next. Setup's
...
rather than silently absorbed. M28a is in flight (Phase 1 complete 2026-08-15)
and M29 remains a registered draft stub.
