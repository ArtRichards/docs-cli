...
one escaping link when the milestone opened. No version bump; **M28** is in flight
and inherits the `BodyLink` span contract plus the guarantee that a clean tree
contains no escaping destination.
**M28's Step 1 (Phases 1–4) is complete (2026-08-15)** on `m28/phases-1-4`,
