...
*Decisions (Phase 1 — BINDING)* with five amendments to setup-frozen material
and nine Step-1 resolutions (OQ-1 … OQ-9, OQ-7 an operator decision), then
authoring 136 test ids and six `archivedate-*` fixture trees against it — 1489
collected, 60 RED and 88 GREEN, no pre-existing id removed or failing, and a
same-instance audit that fixed eight issues and escalated nothing. Step 2
