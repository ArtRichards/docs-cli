...
baseline: **1064 collected, 122 failed, 942 passed**, zero collection errors,
zero tracebacks, zero xfails, exactly two exception classes (105
`AttributeError` through the `_m27()` indirection, 17 `AssertionError`), and
**all 895** pre-existing ids mechanically proven still present and still
passing — `comm -23` and `comm -12 failed old` both **zero**, because M27
removes and modifies no pre-existing test. Each phase is recorded in the
