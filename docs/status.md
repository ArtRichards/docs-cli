...
**M26 — Safe explicit archive selection is the current milestone. Step 1
(Phases 1–4) is complete (2026-08-12 / 2026-08-13, `m26/phases-1-4`) with the
classified RED baseline captured, audited and fresh-eyes reviewed — 884
collected, 104 failed, 780 passed; Phase 5 — Update Base Interfaces is
next.** M26 decouples relationship context from archive
...
- **M26 — Safe explicit archive selection (IN FLIGHT, Step 1 / Phases 1–4
  complete 2026-08-13, all seven setup questions RESOLVED):** unscoped
  related-document writes refuse; preview remains and names the whole
  neighborhood; explicit `--cascade-only` scope is required, planned in full
  before any write; `docs archive --json` emits the operation plan.
...
not transient blockers. M26 now has a full task plan and an implementation log;
M27–M29 remain draft stubs with no implementation logs or phases started.
...
**Next action:** **begin M26 Step 2 with Phase 5 — Update Base Interfaces.**
Phase 1 — Define Contract (2026-08-12) froze the compatibility matrix, the
message catalog, the exit-code split, the `--json` schema, and the Phase-5
signatures in the [M26 plan](m26-safe-archive-selection.md)'s *Decisions
(Phase 1 — BINDING)* section and in `cli.md` / `convention.md`. Phase 2
(2026-08-13) wrote the RED suite — **863 collected, zero collection
errors** — covering E1–E5, the retirement refusals under a 2 × 8 flag matrix,
the unfiltered and filtered previews, the scoped write, every pre-flight
refusal, the injected-`OSError` partial-state admission, and the `--json`
record, and closed two Phase-1 gaps (ineligibility-reason precedence; the
nine-step check order). Phase 3 (2026-08-13) added the four `archive-*`
fixture trees — `archive-neighborhood` (E1), `archive-duplicate-edge` (E2),
`archive-collision` (E3), `archive-archived-neighbour` (E4) — each
`docs check`-clean, and corrected three existing fixtures' prose off the
retired flag. Phase 4 (2026-08-13) captured the classified RED baseline —
**884 collected, 104 failed, 780 passed**, zero collection errors, zero
tracebacks, zero xfails, every failure traced to a family — and proved
mechanically against a throwaway worktree at `37d7f1a` that **769 of the 777
pre-existing ids are still GREEN and the other 8 are the deliberate changes**
(3 removed, 5 failing). One falsely-GREEN `--json` refusal test was caught at
that gate and made honestly RED; the same-instance Step-1 audit found and
fixed eight more issues; and an independent fresh-eyes review found a
**blocker** — a byte-identity assertion that was unsatisfiable because M18
legitimately repoints one `Related:` bullet of an archived neighbour, whose
cheapest "fix" would have been a silent M18 regression — plus six should-fixes,
including a **withdrawn false coverage-boundary claim**: the mid-execution
partial-state admission IS reachable from a subprocess test, via a plan member
at mode `0o644` inside a `0o555` directory. Three conductor decisions were
applied: `--json` gains a top-level `reason` with the candidate field renamed
`exclusion_reason`; `primary.source` is the `FILE` argument exactly as typed;
and `--cascade-only` refuses negated (`!`) patterns. **Phases 1–4 changed no
product code.**
Phase 5 opens Step 2. Each phase is recorded in the
[M26 log](m26-safe-archive-selection-impl.md). M25 is
