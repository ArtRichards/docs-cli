...
**Phase 4** captured the classified RED baseline — **1295 collected, 191
failed, 1104 passed** after the Step-1 same-instance audit's twelve added locks,
zero collection errors, zero xfail/xpass, zero warnings,
exactly two exception classes (156 `AttributeError` through the `_m28()`
indirection, 35 `AssertionError`), every RED in a named family with a
