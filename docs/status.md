...
- pairs-with: archive/2026-08-16/m26-safe-archive-selection-impl.md
...
- pairs-with: archive/2026-08-16/m28a-archive-date-witness-impl.md
...
**docs-cli 2.0.0 shipped to PyPI on 2026-08-16.** M29 — PyPI publish 2.0.0 is
**COMPLETE**: all five runbook phases ran in one session, the same day M28a
merged, publishing the whole v2.0 safety train — M25 (`822e086`), M26
(`393fb53`), M27 (`58955ef`), M28 (`b1ec74b`), M28a (`91cc839`) — as one
breaking release at https://pypi.org/project/docs-cli/2.0.0/.

**Chain of custody is bit-perfect for both artifacts**: the PyPI-served wheel
`bcd8aa45…` and sdist `c62f22d3…` are byte-identical to the local Phase-4
build. The wheel sha was stable across all three builds — Phase 2, the
post-rehearsal canonical rebuild, and Phase 4 — which is the cleanest evidence
that the TestPyPI rehearsal-name detour leaves nothing behind in what ships.
`main` was fast-forwarded `f855b80..408ef6c` and pushed — **101 commits**, the
entire train public in one push, since this project deliberately holds `main`
local between releases — and `v2.0.0` is annotated at `408ef6c`, the commit
those bytes were built from, with the GitHub release carrying the `## 2.0.0`
notes. The host `docs` skill was re-materialised from the published wheel and
is byte-identical to it.

**Acceptance was run identically three times.** The nine v2.0 headline
contracts were scripted once and run unchanged against the local wheel, the
TestPyPI-served wheel and the PyPI-served wheel: **9/9 each time**. Three
manual walkthroughs would have compared three slightly different things.

**Three findings came out of the publish, all recorded.** **D1** was real and
load-bearing: `twine 6.2.0` could not check the artifacts this tree builds,
because an unpinned hatchling now emits `Metadata-Version: 2.5` and twine
monkeypatched `packaging`'s valid list down to `2.4` — a toolchain regression,
not a package defect, caught at *setup* rather than mid-upload. Fixed by
upgrading to twine 7.0.0 and moving the floor into `pyproject.toml`'s `[dev]`
extra so the constraint is version-controlled rather than prose. The
**workflow-skill sweep found real drift for the first time** — nine bare
`--cascade` prescriptions across `create-milestones` and `project-foundation`,
every one the milestone-completion step, all of which now refuse at exit 2;
recorded, not fixed, since the Agent Playbook Suite update is scoped out of M29
as cross-repository work and is now unblocked. And **two runbook instructions
were obsolete** — the twine install-if-absent step, and the claim that the
archive verb does not rebase referring edges, false since M18 and M28 and
disproved by this very closeout — both corrected in place.

**All M29 success criteria are met, the last one just outside the milestone's
own boundary.** Issue #1's prepared reply was re-verified claim by claim
against the **PyPI-served** wheel, but `gh issue comment` was blocked by a
local permission classifier during Phase 5, so it was recorded as unmet rather
than quietly dropped. The operator posted it and **closed issue #1** the same
day (2026-08-16T10:54:25Z). The archived milestone doc still shows that
criterion unticked — correctly, as a snapshot of what was true at closeout; the
convention grants no exception for editing archived prose to tidy it up, which
is exactly why the publish log stays active. The follow-through is recorded
there, under *Post-closeout*.

**The host CLI is upgraded to 2.0.0**, closing a gap this closeout created:
Phase 5 refreshed the host `docs` skill from the published wheel while the host
binary was still 1.8.0, so the host briefly held 2.0 instructions and a 1.x CLI.
Worth carrying forward — the CLAUDE.md policy refreshes host *skills* at
production ship and says nothing about the host *binary*, and nothing detects
the mismatch.

**Next action: none scoped.** The v2.0 train is shipped and archived, and both
cross-repository follow-ons are done: the Agent Playbook Suite is migrated to
2.0 (`cce1699`) and now carries a CI check that fails when suite prose
prescribes a flag the pinned docs-cli has retired (`d22df84`) — the drift lint
that was previously only a candidate. The one item still open is the **PyPI
token re-scope** to project `docs-cli`, rolling forward since M9.

Setup's own readiness pass is what made the publish uneventful. Before any
irreversible step, M29 ran the **non-mutating** half of the runbook — read-only
registry probes, the tree-wide gate, an artifact build into a scratch directory
and a local-install smoke — and recorded seven pieces of evidence (**E1–E7**).
Six confirmed the tree was publish-ready; the seventh was the twine blocker
above. Finding it there rather than mid-upload is the whole argument for the
dry run: the same failure discovered after `main` was pushed and the CHANGELOG
dated would have been a far more awkward thing to unwind.

**M28a — Structured archive-date witness is the milestone just completed, and it is
...
**M29 — PyPI publish 2.0.0** is the only milestone left in the train; it was
activated the same day and its setup is recorded above. Step 1 —
...
the completion summaries are written. A **second `/simplify` pass** then ran on
`m28a/simplify` (2026-08-16) after the audit and the fresh-eyes fold-in and
made **no code changes**. Unlike M28's second pass it had no gap to close:
nothing executable moved after Phase 10 — the whole post-Phase-10 `cli.py`
delta is twelve changed lines (+8 / −4) of prose inside two argparse
`description=` strings — so
Phase 10's analysis is still current and its four rejections still hold. Two
mechanical sweeps confirmed it rather than resting on that: a duplicate-block
scan over **all 3,209 added lines** found exactly one repeated block in the
milestone (the three purity locks' `setattr` scaffolding), and an extended
`ruff` selection the project does not run (`C90,RET,PLR,SIM,C4,PERF,FURB,RUF`)
found exactly one candidate inside M28a's line ranges (`PLR2004` on
`len(parts) < 3`). Both were rejected with reasons, alongside three more, in
the log. **M28a is implementation-complete and
...
to widen the `docs mv` predicate). At that baseline no product code had yet changed; Phases 2–4
...
`config.date_format` so it cannot inherit the bug. The prepared pair is [m28a-archive-date-witness.md](archive/2026-08-16/m28a-archive-date-witness.md) and
[m28a-archive-date-witness-impl.md](archive/2026-08-16/m28a-archive-date-witness-impl.md).
...
the M29 publish closeout; **M28a** followed it and is now
implementation-complete too, so **M29** is next. M28a's milestone setup
completed 2026-08-15 with all seven setup questions RESOLVED, and all ten of
its TDD phases completed without re-opening them — Step 1 on
`m28a/phases-1-4` (contract 2026-08-15; RED tests, fixtures and the classified
baseline 2026-08-16) and Step 2 on `m28a/phases-5-10` (2026-08-16). M28 extends `docs mv` and `docs archive` to rebase the
...
[m28-move-safe-body-link-rewrites.md](archive/2026-08-16/m28-move-safe-body-link-rewrites.md) and
[m28-move-safe-body-link-rewrites-impl.md](archive/2026-08-16/m28-move-safe-body-link-rewrites-impl.md).
...
prepared pair is
[m27-markdown-body-link-validation.md](archive/2026-08-16/m27-markdown-body-link-validation.md)
and
[m27-markdown-body-link-validation-impl.md](archive/2026-08-16/m27-markdown-body-link-validation-impl.md).
...
[m26-safe-archive-selection.md](archive/2026-08-16/m26-safe-archive-selection.md) and
[m26-safe-archive-selection-impl.md](archive/2026-08-16/m26-safe-archive-selection-impl.md).
...
[m25-reciprocal-relationship-integrity.md](archive/2026-08-16/m25-reciprocal-relationship-integrity.md)
and
[m25-reciprocal-relationship-integrity-impl.md](archive/2026-08-16/m25-reciprocal-relationship-integrity-impl.md).
...
- **M28a — Structured archive-date witness (IMPLEMENTATION-COMPLETE — all ten
  TDD phases done, Step 1 on `m28a/phases-1-4` and Step 2 on
  `m28a/phases-5-10`, 1504 passed / 0 failed after the audit and the
  fresh-eyes fold-in; all seven setup questions
  RESOLVED, the contract frozen, and 149 test ids plus six fixture trees
  authored against it; stays `Lifecycle: active` until the M29 closeout):** depends on M26; **two
...
not transient blockers. **All six** now have full task plans and implementation
logs — M29's pair was opened at its 2026-08-16 activation, so no registered
stub remained in the train, and all six are now complete and shipped.
...
**Next action:** none scoped — **`docs-cli==2.0.0` shipped to PyPI 2026-08-16**
and M29 is complete, closing the v2.0 train. The Agent Playbook Suite update and the
workflow-skill drift lint are both done (`cce1699`, `d22df84` in that
repository), and issue #1 is CLOSED. The one item still open is the **PyPI
token re-scope** to project `docs-cli`, rolling forward since M9. M28a is
...
review returned no blockers and the verdict *ship it***, having
mutation-tested the suite on an isolated copy at **19 killed / zero
survivors**, including the three cases designed to fail only on the specific
wrong implementation (parsed-versus-string comparison, raw-versus-re-rendered
message strings, and config-blindness). Its four should-fixes were all
documentation gaps in surfaces no test guarded — most importantly that
`docs archive --help`, the verb that **writes** the field, never mentioned it,
and that `cli.md`'s upgrade note **denied** the `bad-date` residual the
CHANGELOG correctly named. All four, plus six binding nits, are folded in, and
three of them added tests: the `--help` surface-parity lock across all three
verbs, the exclusion-versus-predicate lock, and Leg 2's stderr asserted by
equality. **Phase 7 — Update Tool/Wrapper Layer completed
...
in the [M28 plan](archive/2026-08-16/m28-move-safe-body-link-rewrites.md)'s *Decisions (Phase 1 —
...
[M28 log](archive/2026-08-16/m28-move-safe-body-link-rewrites-impl.md).
...
exception), and the
[M27 plan](archive/2026-08-16/m27-markdown-body-link-validation.md)'s *Decisions (Phase 1 —
...
[M27 log](archive/2026-08-16/m27-markdown-body-link-validation-impl.md). M26 is
...
in the [M26 plan](archive/2026-08-16/m26-safe-archive-selection.md)'s *Decisions (Phase 1 —
...
[M26 log](archive/2026-08-16/m26-safe-archive-selection-impl.md), which carries the milestone
