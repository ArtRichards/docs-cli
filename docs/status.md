...
**M27 — Markdown body-link validation is the current milestone. Step 1 is in
flight on `m27/phases-1-4`; Step 1 is complete — Phases 1–4 (2026-08-14).** M27 adds a pure, stdlib-only, linear scanner over a
...
**Next action:** **begin M27 Step 2 — Phase 5, Update Base Interfaces.**
M27's milestone setup completed 2026-08-14 on
`m27/milestone-setup` with **all seven setup questions resolved**;
**Step 1 is complete — Phases 1–4** (2026-08-14). Phase 1 froze the
...
baseline: **1066 collected, 124 failed, 942 passed** (restated after the
Step-1 same-instance audit), zero collection errors, zero tracebacks, zero
xfails, exactly two exception classes (106 `AttributeError` through the
`_m27()` indirection, 18 `AssertionError`), and **all 895** pre-existing ids
mechanically proven still present and still passing — `comm -23` and
...
frozen in the signature but untested. Each phase is recorded in the
