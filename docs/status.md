...
Milestone setup is complete (2026-08-15) with all seven setup questions
RESOLVED; Phase 1 — Define Contract is next and does not re-open them.** No TDD
phase has started. M28 extends `docs mv` and `docs archive` to rebase the
...
- **M27 — Markdown body-link validation (IMPLEMENTATION-COMPLETE
  ...
  link.
- **M28 — Move-safe body-link rewrites (IN FLIGHT, setup complete 2026-08-15,
  all seven setup questions RESOLVED):** depends on M26 + M27; preserves both incoming
...
**Next action:** **begin M28 Phase 1 — Define Contract.** M28's milestone
setup is complete (2026-08-15 on `m28/milestone-setup`) with a full ten-phase
task plan, an implementation log, eight pieces of measured evidence (E1–E8),
and **all seven setup questions resolved**; no implementation phase has
started. Three of the answers changed something already written down and are
recorded as binding amendments — **A1** narrows the routed strand-check
predicate (its literal form was measured to refuse this repository's own
standard milestone closeout) into a `child-of` refusal plus a report of every
other still-active inbound reference; **A2** supersedes the registered stub's
archived-referrer recommendation with M18's destination-tokens-only shape;
**A3** declines issue #1 finding 4's `--report-links` as a design while
adopting its output as a plan record on both verbs. Phase 1 freezes — against
those resolutions, in the
[M28 plan](m28-move-safe-body-link-rewrites.md)'s *Resolved setup questions
(Q1–Q7, BINDING)*, its *Decisions recorded at setup (BINDING)*, and a new
*Decisions (Phase 1 — BINDING)* section — the three-step rewrite formula and
its move map, the emitted spelling / fragment / delimiter / re-encoding rules,
the byte-for-byte no-op rule, the never-creates-an-escape invariant, the
archived-referrer policy and its `convention.md` wording, both strand-check
legs with their messages and record keys, the pre-flight and partial-state
boundary, and the preview and `--json` shapes for both verbs — then records
Phase 1 in the [M28 log](m28-move-safe-body-link-rewrites-impl.md).
