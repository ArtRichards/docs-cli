...
**M28a — Structured archive-date witness is the current milestone. Phases 1–3
are complete on `m28a/phases-1-4` (contract 2026-08-15; RED tests and fixtures
2026-08-16); Phase 4 — Run Tests (RED Baseline) is next.** All seven setup questions were RESOLVED before Phase 1,
...
(Phase 1 — BINDING)*, with **five amendments** to setup-frozen material and
**nine Step-1 resolutions** (OQ-1 … OQ-9; OQ-7 an operator decision declining
to widen the `docs mv` predicate). No product code has changed; Phases 2–3
authored **131** test ids and **six** `archivedate-*` fixture trees against
that contract, leaving the suite at 1484 collected — **58 RED and 85 GREEN**
of the 143 new ids, with **no** pre-existing id failing. M28a ships **two
...
parsing to different dates), evaluated inside the plan-before-move,
refuse-with-zero-mutation pre-flight **M28 already built**, with both dates
named and zero bytes written. Three neighbours are enumerated as **permitted**
so the predicate cannot creep — a rename within one dated directory, a move
`status-drift` already catches, and a move whose segments do not both parse —
...
seven setup questions RESOLVED, and its **Phase 1 — Define Contract** completed
the same day on `m28a/phases-1-4` without re-opening them. M28 extends `docs mv` and `docs archive` to rebase the
...
- **M28a — Structured archive-date witness (IN FLIGHT, Phases 1–3 complete
  on `m28a/phases-1-4`; all seven setup questions RESOLVED, the contract
  frozen, and 131 RED-baseline test ids plus six fixture trees authored
  against it):** depends on M26; **two
...
**Next action:** **M28a Phase 4 — Run Tests (RED Baseline)**, then
...
items (A)–(H) of *Decisions (Phase 1 — BINDING)*, with **five amendments** to
...
reaches `--dry-run`; and three factual corrections — and **nine Step-1
resolutions** (OQ-1 … OQ-9), of which **OQ-7 is an operator decision** declining
...
changed and the suite was unchanged at **1341 passed**. **Phases 2–3 completed 2026-08-16**:
**131** authored ids across ten test files and **six** `archivedate-*` fixture
trees; 1484 collected, **58 RED / 85 GREEN** of the 143 new ids, and **0**
pre-existing ids failing, with every pre-M28a fixture tree byte-identical. **M28 is complete and merged to `main`**
