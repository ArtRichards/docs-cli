...
in **TDD (draft)** — the follow-on that restores the skill-refresh nudge cut
from M21. **Phases 1–4 (Contract & RED baseline) are complete on branch
`m23/phases-1-4`** (636 tests: 613 pass / 23 intended-RED after the fresh-eyes
fold; contract frozen in
`cli.md` §install-skill + §Update check); Phases 5–10 remain. It
...
**1.8.0** — OQ-1/OQ-2 resolved **provisionally while the operator was away**,
to be confirmed at branch review. The pair is
