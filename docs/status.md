...
- **M28 — Move-safe body-link rewrites:** depends on M26 + M27; preserves both
  incoming links to moved targets and links inside moved referrers, and (added
  2026-08-15) refuses any move whose plan would strand a still-active
...
| M28 — Move-safe Markdown body-link rewrites | **Registered draft** (2026-08-10; no log/phase started; depends on M26 + M27) — incoming-target and moved-referrer link rebasing. **Scope extended 2026-08-15** from issue #1 finding 1: a post-plan **strand-check** refusing any move that would leave a still-active document pointing at a newly-archived one. M26 made archive selection *authorized*, but a glob cannot know what it selects — `--cascade-only '*'` still archives a live `milestone-plan.md` via the target's outgoing `child-of` edge. Validating the plan's consequences is direction-agnostic, self-cancels for whole-set archiving, and does not re-open M26 — Q3. | [Plan](m28-move-safe-body-link-rewrites.md) | _not yet created_ |
