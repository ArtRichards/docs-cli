...
- **M27 — Markdown body-link validation (IMPLEMENTATION-COMPLETE
  ...
  link.
- **M28 — Move-safe body-link rewrites (IN FLIGHT, Step 1 — Phases 1–4 —
  complete 2026-08-15, all seven setup questions RESOLVED):**
...
- **M28a — Structured archive-date witness:** depends on M26; gives an archived
  document a structured record of its own archive date so a relocated one is
...
not transient blockers. M26, M27 and M28 all have full task plans and
implementation logs; M28a and M29 remain draft stubs with no implementation
logs or phases started.
...
exit 0. **Phase 2** wrote **182** new ids — a 153-item pure-planner module plus
12 `mv`, 14 `archive`, 1 `check` and 2 bundled-skill locks — with zero deleted
test lines and zero collection errors (1269 collected, 181 failed, 1088
passed). **Phase 3** authored the seven `movelink-*` fixture trees, each
...
**Phase 4** captured the classified RED baseline — **1283 collected, 180
failed, 1103 passed**, zero collection errors, zero xfail/xpass, zero warnings,
exactly two exception classes (152 `AttributeError` through the `_m28()`
indirection, 28 `AssertionError`), every RED in a named family with a
...
mechanically proven present and passing (`comm -23` = 0, `comm -12 failed old`
= 0). No product code was touched across Phases 1–4, by design. The phase
records live in the
