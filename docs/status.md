...
**M27 — Markdown body-link validation is the current milestone. Milestone
setup is complete (2026-08-14); Phase 1 — Define Contract is next.** No TDD
phase has started. M27 adds a pure, stdlib-only, linear scanner over a
...
**no fixture needs updating**. Phase 1 does not re-open any of this: it freezes
the grammar, the masking and containment contracts, both message templates,
and the `BodyLink` span record. The package version stays **1.8.0**
...
- **M27 — Markdown body-link validation (IN FLIGHT, setup complete
  2026-08-14, all seven setup questions RESOLVED):** a bounded stdlib-only
...
**Next action:** **begin M27 Phase 1 — Define Contract.** M27's milestone
setup is complete (2026-08-14 on `m27/milestone-setup`) with **all seven setup
questions resolved**; no implementation phase has started. Phase 1 must
freeze — against those resolutions, in the
[M27 plan](m27-markdown-body-link-validation.md)'s *Resolved setup questions
(Q1–Q7, BINDING)* — the supported Markdown grammar subset and its exactness
rules, the length-preserving code-masking contract, destination classification
and document-relative resolution, **the containment test** (join → lexical
canonicalisation → under-root membership, with the `..`-escape-then-return and
symlink cases decided) and **its precedence over the existence test** so
...
and the `BodyLink` destination-span contract M28 consumes — in `cli.md`
(including the explicitly stated out-of-root boundary) and `convention.md`
(the inside-the-root invariant, fence-your-code-samples, and the third
archived-document exception) — then record Phase 1 in the
