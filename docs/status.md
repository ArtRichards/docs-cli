...
**One success criterion is outstanding.** Issue #1's prepared reply was
re-verified claim by claim against the **PyPI-served** wheel, but the comment
was **not posted**: `gh issue comment` was blocked by a local permission
classifier. The verified body sits in the archived milestone doc's *Issue #1
closeout* block; posting it and closing the issue is left to the operator. It
is recorded as unmet rather than quietly dropped.

**Next action: none scoped.** The v2.0 train is shipped and archived. The named
follow-ons are the Agent Playbook Suite update (unblocked by this release), the
in-repo workflow-skill drift lint, and the PyPI token re-scope that has rolled
forward since M9.
...
and M29 is complete, closing the v2.0 train. The named follow-ons are the Agent
Playbook Suite update (nine retired-`--cascade` prescriptions, unblocked now
that 2.0.0 is available), the in-repo workflow-skill drift lint, issue #1's
verified-but-unposted reply, and the PyPI token re-scope rolling forward since
M9. M28a is
