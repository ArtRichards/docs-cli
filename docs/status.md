...
**M29 — PyPI publish 2.0.0 is the current milestone. Milestone setup completed
2026-08-16; no runbook phase has started.** M29 is operational rather than a
ten-phase code milestone — the sections of
[release-runbook.md](release-runbook.md) are its phases — and it is now
`Lifecycle: active` with
[m29-pypi-publish-2-0-0-impl.md](m29-pypi-publish-2-0-0-impl.md) opened beside
it. Every milestone it ships is merged to `main`: M25 (`822e086`), M26
(`393fb53`), M27 (`58955ef`), M28 (`b1ec74b`), M28a (`91cc839`).

Setup ran the **non-mutating** half of the runbook's Phase 1 and Phase 2 gates
against the merged tree — read-only registry probes, the tree-wide quality
gate, an artifact build into a scratch directory, and a local-install smoke —
and recorded seven pieces of evidence (**E1–E7**) in the log. Nothing was
uploaded, pushed, tagged or renamed, and the repository's `dist/` still holds
the shipped 1.8.0 artifacts untouched. **Six of the seven say go**: the `2.0.0`
slot is free on PyPI and on the `docs-cli-rehearsal` detour name (the bare
TestPyPI name is still squatted, unchanged since M9), `~/.pypirc` and the `gh`
`repo` scope are intact, the gate is green at **1504 passed** with `ruff`,
`ruff format`, `mypy`, `docs check`, index idempotence and INDEX-vs-fixture all
clean, and **all eight v2.0 headline contracts hold against a locally built
wheel** installed into a throwaway venv — `missing-inverse`, the bare
`--cascade` refusal at exit 2 with zero writes, `broken-body-link`, `docs mv
--dry-run`'s full rewrite plan, the archive strand report, the `Archived:`
witness on every moved member, the dated-archive-directory `mv` refusal, and
`archive-date-drift`.

**The seventh is a blocker, and it is in the toolchain rather than the
package.** `twine 6.2.0` cannot check the artifacts this tree now builds: an
unpinned hatchling emits `Metadata-Version: 2.5`, and twine monkeypatches
`packaging`'s valid-version list to stop at `2.4`, so `twine check` errors on
both wheel and sdist. The installed `packaging 26.2` accepts `2.5` on its own,
**PyPI accepts `2.5`** (hatchling's own wheel, uploaded 2026-08-11, serves it),
and `twine 7.0.0` — verified in a throwaway venv — **PASSES** both of today's
artifacts. Recorded as **D1**: upgrade to `twine>=7.0.0` at Phase 2; pinning
hatchling backwards is explicitly rejected. The stub's three activation
questions are RESOLVED as **D2** (M24's "author now, confirm at the gate" —
upload, `main` push, tag, release and the issue #1 comment each stop for
explicit operator authorization), **D3** (the closeout archives the five
plan/log pairs plus the M29 milestone doc — eleven documents — while the M29
log stays active at the root on the M17/M20/M24 precedent) and **D4** (the
README's `blob/main/…` links are re-verified *after* the closeout archive; the
rest of the README refresh ships in Phase 2 because `README.md` is the PyPI
project page). Setup also found a **fourth** README item beyond the three M28
routed here: three surviving "Claude Code skill" mentions that M23's
neutralisation never swept, in the one file that is outside every parity gate
and is also the PyPI front page.
...
stub remains in the train; M29 has no runbook phase started.
...
**Next action:** **M29 Phase 1 — Operator one-time prep**, then Phase 2's
`twine` upgrade (D1) and the single version bump. M29 is **active**, its setup
complete 2026-08-16 with all three activation questions resolved (D1–D4) and
its readiness measured (E1–E7); the whole v2.0 train (M25–M28a) is merged to
`main`, so nothing but the publish itself is outstanding. M28a is
...
| M29 — PyPI publish 2.0.0 | **Active — milestone setup complete 2026-08-16; no runbook phase started** (registered 2026-08-10, activated on `m29/milestone-setup` the day M28a merged; depends on M25–M28a, all five merged to `main`). Operational rather than ten-phase — `release-runbook.md`'s sections are its phases, written out as a five-phase plan with exit criteria. Setup ran the non-mutating half of the runbook's Phase 1 and Phase 2 gates and recorded **E1–E7**: the `2.0.0` slot is free on PyPI and on the `docs-cli-rehearsal` detour name (the bare TestPyPI name is still squatted), credentials and `gh` scopes intact, the gate green at **1504 passed**, all **eight** v2.0 headline contracts holding against a locally built wheel, and the version SoT still `1.8.0` by design. **One blocker, in the toolchain not the package:** `twine 6.2.0` rejects the `Metadata-Version: 2.5` artifacts an unpinned hatchling now builds, while PyPI accepts 2.5 and `twine 7.0.0` passes both — **D1** upgrades the checker. Activation questions RESOLVED as **D2** (operator authorization at the irreversible gates), **D3** (eleven-document closeout manifest; the M29 log stays active) and **D4** (README links re-verified after the closeout archive). | [Plan](m29-pypi-publish-2-0-0.md) | [Log](m29-pypi-publish-2-0-0-impl.md) |
